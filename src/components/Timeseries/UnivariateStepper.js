...
export default function UnivariateStepper() {
  ...
  const getStepContent = step => {
    switch (step) {
      case 0:
        return <DataSelectionStep fetched={fetched} handleNext={handleNext} handleBack={handleBack} handleURLChanged={handleURLChanged} handleTypeChanged={handleTypeChanged} type={type} url={url} handleFetch={handleFetch} tsData={tsData} />;
      case 1:
        return (
          <TrainContainer
            tsdata={tsData[0]} predicted={predicted} handleNext={handleNext} handleBack={handleBack} setPredicted={setPredicted} setPredictions={setPredictions}
          />
        );
      case 2:
        return (
          <PredictContainer handleBack={handleBack} handleReset={handleReset} predicted={predicted} predictions={predictions} />
        );
      default:
        return "Error";
    }
